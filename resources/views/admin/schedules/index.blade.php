...
@section('js')
    ...
    <script>
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $('#ModalCenter form').on('submit', function(e) {
                        e.preventDefault();
                        // Verificar ayudantes seleccionados
                        var ayudantes = $('#ayudantes_ids').val();
                        if (!ayudantes || ayudantes.length === 0 || ayudantes[0] === '') {
                            Swal.fire({
                                icon: 'error',
                                title: 'Error',
                                text: 'Debe seleccionar al menos un ayudante disponible para registrar la programación.'
                            });
        ...
        $(document).on('click', '.btnShow', function() {
            var id = $(this).attr("id");
            $.ajax({
                url: "{{ route('admin.schedules.show', 'id') }}".replace('id', id),
                type: "GET",
                success: function(response) {
                    $('.modal-title').html("Detalle de la programación");
                    $('#ModalCenter .modal-body').html(response);
                    $('#ModalCenter').modal('show');
                }
            })
